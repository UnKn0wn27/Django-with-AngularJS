...
angular.module('core.comment')
  .directive('commentReplyThread', function(Comment, $cookies){
    return {
      ...
      template: "<div class='row' ng-repeat='r in replies' style='margin-top: 10px;'>" +
                  "<div class='col-sm-12'>" +
                    "<div class='panel panel-default'>" +
                      "<div class='panel-body'>" +
                        ...
                        "<a href='#' confirm-click='Do you want to delete this?'" +
                        "confirmed-click='deleteComment(r, comment)'>Remove</a></small>" +
                      "</div>" +
                    "</div>" +
                  "</div>" +
                "</div>" +
                "<div class='text-center' ng-show='!replies'>" +
                  "<img style='margin: 0 auto;' ng-src='/static/img/Pacman.gif' class='img-responsive'/>" +
                "</div>" +
                ...
                "<form ng-submit='addCommentReply(reply, comment)'>" +
                  "<div class=\"form-group\" ng-class=\"{'has-error': replyError.content}\">" +
                    "<textarea class='form-control' id='replyText-{{comment.id}}'" +
                      "ng-model='reply.content' placeholder='Write reply'></textarea>" +
                    ...
                    "</label>" +
                  "</div>" +
                  "<input class='btn btn-small' type='submit' value='Reply'/>" +
                "</form>",
